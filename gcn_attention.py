...
opt_g = optim.Adam(params=model.parameters(), lr=lr)
opt_m1 = optim.Adam(params=mlp1.parameters(), lr=lr)
opt_m2 = optim.Adam(params=mlp2.parameters(), lr=lr)
opt_m3 = optim.Adam(params=mlp3.parameters(), lr=lr)
# opt_m4 = optim.Adam(params=mlp4.parameters(), lr=lr)
...
for epoch in range(num_epochs):
    ...
    epoch_loss = 0
    step = 0
    while(samples_read<len(train_data)):
        ...
        # obtain batch outputs
        batch = train_data[samples_read:min(samples_read+batch_size,len(train_data))]
        cur_batch_size = len(batch)
        samples_read += cur_batch_size

        term_hiddens, y, out_len, node_embeddings, batch_vocabs, encoder_hidden_list, seq_nums = embed_batch(batch)

        # if teach_force_rate != 0 and epoch % (int(1/teach_force_rate)) == 0:
        #     with_truth = True
        ...
        # out = decode_batch(term_hiddens, y, with_truth, node_embeddings, batch_vocabs)
        # out = decode_batch(term_hiddens, y, with_truth, None, None, encoder_hidden_list, seq_nums)
        out = decode_batch(term_hiddens, y, with_truth, None, None, encoder_hidden_list, seq_nums)
        # out.register_hook(save_grad('out'))


        target = pack_padded_sequence(y,out_len.tolist(), batch_first=True, enforce_sorted=False)[0]
        pad_out = pack_padded_sequence(out,out_len.tolist(), batch_first=True, enforce_sorted=False)[0]

        # #防止log前出现0
        # pred_mask = (pad_out == 0).float() * 1e-40 
        # print(torch.sum(pred_mask))
        # pad_out = pad_out + pred_mask
        pad_out = pad_out + 1e-4
        # pad_out.register_hook(save_grad('pad_out_pre'))
        pad_out = torch.log(pad_out)
        # pad_out.register_hook(save_grad('pad_out'))
        loss = criterion(pad_out, target)
        loss.backward()
        # if step == 144:
        #     print(grads['pad_out_pre'], grads['pad_out'])
        # print(step, loss.item(), samples_read)
        # if torch.sum(grads['pad_out_pre'] != grads['pad_out_pre']) > 0:
        #     print('pad_out_pre'+'出现nan')
        # if torch.sum(grads['pad_out'] != grads['pad_out']) > 0:
        #     print('pad_out'+'出现nan')
        # if torch.sum(grads['gcn_input'] != grads['gcn_input']) > 0:
        #     print('gcn_input'+'出现nan')
        # if torch.sum(grads['term_hiddens'] != grads['term_hiddens']) > 0:
        #     print('term_hiddens'+'出现nan')
        # if torch.sum(grads['out'] != grads['out']) > 0:
        #     print('out'+'出现nan')
        # #梯度裁剪
        # torch.nn.utils.clip_grad_norm_(encoder.parameters(), 5)
        # torch.nn.utils.clip_grad_norm_(model.parameters(), 5)
        # # torch.nn.utils.clip_grad_norm_(mlp1.parameters(), 5)
        # torch.nn.utils.clip_grad_norm_(mlp2.parameters(), 5)
    ...
    # teach_force_rate=teach_force_rate*0.98
    # if teach_force_rate<0.7:
    #     teach_force_rate=0.7
    k = 12  #小数据集16大数据集12
    teach_force_rate=k/(k+np.exp(epoch/k))
    print('teach_force_rate:', teach_force_rate)
    if teach_force_rate<0.2:
        teach_force_rate=0.2

    print('该轮迭代损失：', epoch_loss/step)
    if (epoch+1) % eval_epoch == 0:
        with torch.no_grad(): #减少内存占用，pytorch不用存储计算图
            ...
            save_path = abs_file_path + '/results/' + nowTime + '_epoch-' + str(epoch+1) + '.txt'
            save_results(pred_text, ground_truth, score_format_list, save_path)
    #保存模型
    if ((epoch+1)%5== 0):
        model_save_path = abs_file_path + '/models/' + str(epoch)
        if not os.path.exists(model_save_path):
            os.makedirs(model_save_path)
        torch.save(encoder, model_save_path+'/encoder')
        torch.save(model, model_save_path+'/gcn')
        torch.save(mlp1, model_save_path+'/mlp1')
        torch.save(mlp2, model_save_path+'/mlp2')
        torch.save(mlp3, model_save_path+'/mlp3')
        # torch.save(mlp4, model_save_path+'/mlp4')
        torch.save(decoder, model_save_path+'/decoder')
